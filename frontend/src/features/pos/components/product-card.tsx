...
export function ProductCard({ product, onAdd }: ProductCardProps) {
  ...
  return (
    <button
      onClick={handleClick}
      className={`relative w-full text-left border rounded-lg p-3 bg-card hover:bg-accent/10 hover:border-primary/40 transition-all duration-150 cursor-pointer group active:scale-[0.97] ${added ? 'border-primary bg-primary/5' : ''}`}
    >
      {added && (
