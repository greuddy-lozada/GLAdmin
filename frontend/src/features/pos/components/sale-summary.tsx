  ...
  return (
    <div className="mt-4 border rounded-lg p-4 space-y-1 bg-muted/30">
      <div className="flex justify-between text-sm text-muted-foreground">
        ...
        <span className="tabular-nums">${(total - totalTax).toFixed(2)} / ${(totalUsd - totalTaxUsd).toFixed(2)}</span>
      </div>
      {totalTax > 0 && (
        <div className="flex justify-between text-sm text-muted-foreground">
          <span>{t('pos.cart.tax')}</span>
          <span className="tabular-nums">${totalTax.toFixed(2)} / ${totalTaxUsd.toFixed(2)}</span>
        </div>
      )}
      {hasWithholding && (
        <div className="flex justify-between text-sm text-destructive">
          <span>{tp('pos.withholding.rate', { percentage: String(withholdingPercentage) })}</span>
          <span className="tabular-nums">-${(withholdingAmount ?? 0).toFixed(2)} / -${(withholdingAmountUsd ?? 0).toFixed(2)}</span>
        </div>
      )}
      <div className="flex justify-between text-lg font-bold pt-2 border-t">
        <span>{t('pos.cart.totalVes')}</span>
        <span className="tabular-nums text-primary">${(total + totalTax).toFixed(2)}</span>
      </div>
      {(totalUsd + totalTaxUsd) > 0 && (
        <div className="flex justify-between text-sm">
          <span>{t('pos.cart.totalUsd')}</span>
          <span className="tabular-nums text-muted-foreground">${(totalUsd + totalTaxUsd).toFixed(2)}</span>
        </div>
      )}
      {hasWithholding && (
        <div className="flex justify-between text-base font-semibold pt-1 border-t border-dashed">
          ...
          <span className="tabular-nums text-primary">${(netToCollect ?? total + totalTax).toFixed(2)} / ${(netToCollectUsd ?? totalUsd + totalTaxUsd).toFixed(2)}</span>
        </div>
      )}
      {exchangeRate > 0 && (
