...
```bash
pnpm --filter frontend exec react-doctor rules explain react-doctor/no-array-index-as-key
```
...
```bash
pnpm --filter frontend exec react-doctor rules list
pnpm --filter frontend exec react-doctor rules list --configured
pnpm --filter frontend exec react-doctor rules list --category Performance
pnpm --filter frontend exec react-doctor rules explain <rule>
pnpm --filter frontend exec react-doctor rules disable <rule>
pnpm --filter frontend exec react-doctor rules enable <rule>
pnpm --filter frontend exec react-doctor rules set <rule> warn
pnpm --filter frontend exec react-doctor rules category "React Native" off
pnpm --filter frontend exec react-doctor rules ignore-tag design
pnpm --filter frontend exec react-doctor rules unignore-tag design
```
...
- **User disagrees with one rule / it's a false positive for them** → `rules disable <rule>` (sets `rules.<key> = "off"`; the rule never runs). This is the default for "I don't want this rule".
...
Config lives in `frontend/doctor.config.ts` (or `.js`/`.mjs`/`.cjs`/`.json`/`.jsonc`), or the `reactDoctor` key in `frontend/package.json`. The `rules` commands edit whichever exists — TS/JS edits preserve formatting (via magicast) — and create `doctor.config.json` when none does, stamping `$schema`:
